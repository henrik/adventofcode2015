...
DATA.read.each_line do |line|
  num = line[/Sue (\d+)/, 1].to_i
  data = line.split(": ", 2).last.split(", ").map { |kv| k, v = kv.split(": "); [ k.to_sym, v.to_i ] }.to_h
  sues[num] = data
end

p sues.find_all { |num, known_facts|
  known_facts.all? { |k, v| CLUES[k] == v }
}
